...
(function () {
    ...
    const DotsToTrimers = {};
    const Trimers = {};
    const NearestTrimers = new PriorityQueue((a, b) => Math.pow((a.coord[0] - 20), 2) + Math.pow((a.coord[1] - 20), 2) < Math.pow((b.coord[0] - 20), 2) + Math.pow((b.coord[1] - 20), 2));
    ...
    canvas.onclick = e => {
        ...
        let orthP = [Math.floor(orth[0]), Math.floor(orth[1])];
        let trimer = { upper: orth[0] + orth[1] - orthP[0] - orthP[1] > 1, coord: copy(orthP) };
        commitTrimer(trimer);
        ctx.fillStyle = "pink";
        drawTrimer(trimer);
        /*
    ...
    */
    ...
    function workIteration() {
        ...
        for (const x of iterateAdjDots()) {
            const filtered = getSurroundingTrimers(x).filter(y => 
